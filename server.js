...
app.get("/teambuilder.html", function(req, res) {
    res.render("teambuilder.kiwi", {
        load: req.query.load === "true"
    });
});
...
setRouter.get("/:gen/:pokemon", function(req, res) {
    if (!sets.hasOwnProperty(req.params.gen)) {
        res.json({ "error": "Gen not found: " + req.params.gen });
    } else if (!sets[req.params.gen].hasOwnProperty(req.params.pokemon)) {
        res.json({
            "error": "Pokemon not found in gen " + req.params.gen +
            ": " + req.params.pokemon
        });
    } else {
...
let replayRouter = express.Router();
